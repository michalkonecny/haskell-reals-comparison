...
import qualified AERN2.MP as AERN2MP
import qualified AERN2.MP.Ball as AERN2MPBall
    ...
    resultDecription =
        case (take (length "manydigits") benchS, benchParams) of
            ("logistic", [n]) ->
                case implS of
                    "cdar_mBound" -> 
                      CDAR.showA . CDAR.limitSize (int acD) . CDAR.require (int acD) $ (TP.taskLogistic n)
...
taskMBfromTask ac task =
    case snd $ last $ AERN2MP.iterateUntilAccurate ac withP of
      Just r -> r
      _ -> error "taskMBfromTask failed"
    where
    withP p = 
      task (Just . mpBallP p) (Just $ AERN2MPBall.piBallP p) (Just . AERN2MP.setPrecision p)
    -- checkAccuracy
...
taskLogisticCRpureArrow_TA :: Integer -> AERN2Real.CReal
taskLogisticCRpureArrow_TA n =
  TA.taskLogistic n $ AERN2Real.creal (TP.taskLogistic_x0 :: Rational)
...
taskLogisticMP_TP n ac =
    snd $ last $ AERN2MP.iterateUntilAccurate ac withP
    where
    withP p =
...
checkAccuracyMP ball
    | AERN2MP.getAccuracy ball < (AERN2MP.bits 100) = Nothing
    | otherwise = Just ball
...
-- instance AERN2MP.HasPrecision IReal where

-- instance AERN2MP.HasAccuracy IReal where
--   getAccuracy rB
--     | rdIsZero = AERN2MP.Exact
--     | otherwise =
